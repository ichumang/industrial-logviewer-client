...
### Added
- **Dynamic schema engine** — runtime schema negotiation via description request/reply protocol; zero hardcoded data structures
- **Schema filter dialog** — user can choose which fields to display per data type (persisted per session)
...
- **Activation data pipeline** — full CMD request/reply cycle for activation events including Aktiv/Inaktiv status labeling
...
### Changed
- Parser is now fully data-driven from server-declared field descriptors
- Data request padded to 60 bytes as per server protocol specification
...
### Known Limitations
- Categories `Fahrdaten`, `Programmierung`, `Antriebsfehler`: parsing engine ready, GUI activation pending server command specification
...
### Added
- Full activation data category (days request + data request pipeline)
...
- Pult (console) selector dropdown (default: Pult 1)
- Year selector input
- Hierarchical date tree explorer (year → month → day)
- Right-panel content viewer with header
- Schema / filter dialog skeleton
...
- i18n framework — runtime language switching via `lang/*.json`
- German (de) and English (en) language packs
...
- Status bar with request/reply statistics
